"""Training loop for the physics-informed model."""
...
def run():
    device = get_device()
    print(f"Running physics-informed training on {device}")
    ...
    net = UNet(in_channels=1, out_channels=1, base_channels=model.unet_channels).to(device)
    ...
    for epoch in range(1, training.epochs + 1):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch}/{training.epochs} [train]", unit="batch") as pbar:
            for batch in pbar:
                ...
                kappa_pred = net(lensed)
                lensed_pred = simulate_lensed_image(kappa_pred, source)

                loss_kappa = mse(kappa_pred, kappa)
                loss_image = mse(lensed_pred, lensed)
                loss = loss_kappa + training.lambda_physics * loss_image
        ...
        with torch.no_grad(), tqdm(val_loader, desc=f"Epoch {epoch}/{training.epochs} [val]", unit="batch") as pbar:
            for batch in pbar:
                ...
                loss_image = mse(lensed_pred, lensed)
                loss = loss_kappa + training.lambda_physics * loss_image
                running_val_loss += loss.item() * lensed.shape[0]
        ...
        print(
            f"Epoch {epoch:03d}/{training.epochs:03d}  train_loss={epoch_train_loss:.5f}  val_loss={epoch_val_loss:.5f}"
        )
    ...
    ckpt_path = os.path.join(training.checkpoint_dir, "physics_informed.pth")
    torch.save(net.state_dict(), ckpt_path)
    print(f"Saved checkpoint: {ckpt_path}")

    plot_loss_curve(
        {"train": train_losses, "val": val_losses},
        os.path.join(training.plot_dir, "physics_loss.png"),
    )
...
def _save_visualizations(net: nn.Module, dataset: LensingDataset, device: torch.device, epoch: int):
    ...
    out_dir = os.path.join(training.plot_dir, "physics")
    os.makedirs(out_dir, exist_ok=True)

    plot_image_grid(
        [lensed[0], kappa_true[0], kappa_pred[0], lensed_pred[0]],
        ["Lensed", "True κ", "Pred κ", "Re-lensed"],
